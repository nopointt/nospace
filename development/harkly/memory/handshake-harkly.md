...
`2026-03-07` Короткая консультационная сессия. LinkedIn API — закрыт/дорогой, не подходит для Steam indie outreach. Домен: .ru/.kz — плохо для US/EU deliverability; рекомендация .com (Namecheap $10) или .io + transfer в CF Registrar. Figma MCP пакет исправлен: @figma/mcp-server → figma-developer-mcp. | следующее: зарегать домен → Brevo настроить → тексты 12 вариантов
