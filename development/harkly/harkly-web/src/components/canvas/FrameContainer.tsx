...
import type { FrameData, GroupTransform, GroupRect } from "~/lib/canvas/types";
import {
    WINDOW_DEFAULT_X,
    WINDOW_DEFAULT_Y,
...
// --- Shared window event delegation ---
// Instead of each FrameContainer instance adding its own window mousemove/mouseup
...
interface FrameContainerProps {
    ...
    initialRect?: GroupRect | null;
}

const FrameContainer: Component<FrameContainerProps> = (props) => {
    const [pos, setPos] = createSignal({
        x: props.data.x || WINDOW_DEFAULT_X,
        y: props.data.y || WINDOW_DEFAULT_Y,
    });
    const [size, setSize] = createSignal({
        width: props.data.width || WINDOW_DEFAULT_WIDTH,
        height: props.data.height || WINDOW_DEFAULT_HEIGHT,
    });
    ...
    createEffect(() => {
        setZIndex(props.isActive ? 100 : 10);
    });

    // Sync props -> signals when not interacting
    createEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        rafId = requestAnimationFrame(() => {
            rafId = null;
            const worldX =
                (clientX - props.viewport.x) / props.viewport.zoom;
            const worldY =
                (clientY - props.viewport.y) / props.viewport.zoom;
            ...
            if (isDragging()) {
                ...
                setPos(newPos);
                props.onInteractionUpdate?.(props.data.id, {
                    ...newPos,
                    ...size(),
                });
            } else if (isResizing()) {
                ...
                if (dir?.includes("e")) {
                    setSize((prev) => ({
                        ...prev,
                        width: Math.max(WINDOW_MIN_SIZE, worldX - pos().x),
                    }));
                }
                if (dir?.includes("s")) {
                    setSize((prev) => ({
                        ...prev,
                        height: Math.max(WINDOW_MIN_SIZE, worldY - pos().y),
                    }));
                }
                if (dir?.includes("w")) {
    ...
    return (
        <div
            ...
            style={{
                ...
                height: isMinimized()
                    ? "32px"
                    : `${visualSize().height}px`,
                "z-index": zIndex(),
            }}
            onMouseDown={(e) => props.onActivate?.(e)}
            onDblClick={(e) => {
                e.stopPropagation();
                props.onDoubleClick?.(props.data.id);
            }}
        >
            {/* Resize Handles */}
            {!isMinimized() && (
                <>
                    <div
                        class="absolute -top-1 -left-1 w-3 h-3 cursor-nw-resize z-50"
                        onMouseDown={(e) => handleResizeStart(e, "nw")}
                    />
                    <div
                        class="absolute -top-1 -right-1 w-3 h-3 cursor-ne-resize z-50"
                        onMouseDown={(e) => handleResizeStart(e, "ne")}
                    />
                    <div
                        class="absolute -bottom-1 -left-1 w-3 h-3 cursor-sw-resize z-50"
                        onMouseDown={(e) => handleResizeStart(e, "sw")}
                    />
                    <div
                        class="absolute -bottom-1 -right-1 w-3 h-3 cursor-se-resize z-50"
                        onMouseDown={(e) => handleResizeStart(e, "se")}
                    />
                    <div
                        class="absolute top-0 left-0 right-0 h-1 cursor-n-resize z-40"
                        onMouseDown={(e) => handleResizeStart(e, "n")}
                    />
                    <div
                        class="absolute bottom-0 left-0 right-0 h-1 cursor-s-resize z-40"
                        onMouseDown={(e) => handleResizeStart(e, "s")}
                    />
                    <div
                        class="absolute top-0 bottom-0 left-0 w-1 cursor-w-resize z-40"
                        onMouseDown={(e) => handleResizeStart(e, "w")}
                    />
                    <div
                        class="absolute top-0 bottom-0 right-0 w-1 cursor-e-resize z-40"
                        onMouseDown={(e) => handleResizeStart(e, "e")}
                    />
                </>
            )}

            <div class="flex flex-col h-full bg-h-surface border border-h-border-s rounded-lg overflow-hidden shadow-md">
                {/* Header */}
                <div
                    ...
                    <div class="flex items-center justify-between w-full overflow-hidden">
                        ...
                        <div class="flex items-center gap-2 shrink-0 ml-2">
                            <button
                                ...
                                class="text-h-text-3 hover:text-h-text-2 transition-colors"
                                title="Minimize"
                            >
                                <svg
                                    width="14"
                            ...
                            <button
                                ...
                                class="text-h-text-3 hover:text-h-red transition-colors"
                                title="Close"
                            >
                                <svg
                                    width="14"
                ...
                {!isMinimized() && (
                    <div class="flex-1 overflow-hidden relative">
                        <Show
                            ...
                            fallback={
                                <div class="p-4 text-h-text-3 text-sm font-mono">
                                    Unknown frame type: {props.data.type}
                                </div>
                            }
                        >
                            <ErrorBoundary>
                                <Dynamic
                                    component={FRAME_REGISTRY[frameType()]}
                                    data={props.data}
                                />
                            </ErrorBoundary>
                        </Show>
