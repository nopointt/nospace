...
| epic-eidolon-v1 | omnibar | Claude Code integration as Eidolon AI backend | **OPEN** — Eidolon работает end-to-end, системный промпт, markdown, bug hunt sprint 1 завершён |
...
|---|---|---|
| project_phase | omnibar-v1 stabilization: Eidolon работает, bug hunt sprint 1 done (26/39 fixes) | 2026-03-10 |
...
| claude_bridge | tlos-claude-bridge (Node.js) — WORKING: stdin prompt, session persistence, system prompt injection | 2026-03-10 |
| dispatcher | tlos-dispatcher — FIXED (graceful wasm skip, no crash on missing math_worker.wasm) | 2026-03-08 |
| eidolon | Eidolon AI backend LIVE — system prompt (5923 chars), session persistence (~/.tlos/sessions.json), context tracking | 2026-03-10 |
| markdown_chat | Chat.tsx renders markdown via `marked` — bold=yellow, code=cyan, GFM+breaks | 2026-03-10 |
...
|---|---|---|---|
| ~~claude-bridge: cmd.exe breaks on <SYSTEM_CONTEXT> in -p arg~~ | claude-bridge | 2026-03-10 | **RESOLVED 2026-03-10** — prompt via stdin (`--print` + child.stdin.write), no shell arg escaping |
| ~~Neural Link frame спавнится на каждое сообщение~~ | frontend | 2026-03-10 | **RESOLVED 2026-03-10** — добавлен фильтр agent:* events в useComponents.ts |
...
- **Claude AI pipeline:** Omnibar → agent:chat{provider:'claude', model} → NATS → claude-bridge → `claude --print` (stdin) → NATS → Omnibar
  - System prompt: `<SYSTEM_CONTEXT>` injected on first message of new session (5923 chars)
  - Session persistence: `~/.tlos/sessions.json` (sessionId → claudeSessionId)
  - Context tracking: agent:context event после result (tokensUsed/contextTotal)
...
- **Markdown:** Chat.tsx использует `marked` (GFM+breaks), стили в index.css (.chat-md)
...
| Context bar: real token counts from agent:context | TODO |
...
3. FEATURE: Context bar real token counts
