 ...
 * Formula: Profitability = УТН% × (365 / ТЗ_days) × (Кред / (Кред + Деб))
 * Where:
 *   УТН% = realized markup
 *   ТЗ_days = days of inventory (DOI)
 *   Кред = creditor days (payment deferral from suppliers)
 *   Деб = debtor days (marketplace payout delays)
 *
 * Simplified for demo: Profitability = УТН% × (365 / DOI)
 * Full formula with credit leverage: × (credDays / (credDays + debDays))
 */
 ...
 * @param regulatedSharePct - % of revenue from regulated drugs
 * @param regulatedMarkupPct - average markup on regulated drugs
 * @param freeMarkupPct - markup on free-priced drugs (e.g. 30%)
 */
